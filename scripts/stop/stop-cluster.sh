...
# Usage: ./scripts/stop/stop-cluster.sh  (run from project root)
#
...
# Resolve project root (two directories up from scripts/stop/)
...
if kubectl get namespace smartfinvo &>/dev/null; then
  kubectl delete -f "$SCRIPT_DIR/kubernetes/base/deployment.yaml" --ignore-not-found=true
  kubectl delete -f "$SCRIPT_DIR/kubernetes/base/service.yaml"    --ignore-not-found=true
  kubectl delete -f "$SCRIPT_DIR/kubernetes/base/hpa.yaml"        --ignore-not-found=true
  kubectl delete -f "$SCRIPT_DIR/kubernetes/base/postgres/"  --ignore-not-found=true
  kubectl delete -f "$SCRIPT_DIR/kubernetes/base/redis/"     --ignore-not-found=true
  kubectl delete -f "$SCRIPT_DIR/kubernetes/base/configmap.yaml" --ignore-not-found=true
  kubectl delete -f "$SCRIPT_DIR/kubernetes/base/secret.yaml"    --ignore-not-found=true
...
cd "$SCRIPT_DIR/terraform/infrastructure"
...
echo "╚══════════════════════════════════════════╝"
echo ""
echo "Run ./scripts/start/start-cluster.sh when you want to start again (~10 min)."
echo ""
...
Run ./scripts/start/start-cluster.sh to bring the cluster back up (~10 min)." \
    --region us-east-2 > /dev/null
fi
