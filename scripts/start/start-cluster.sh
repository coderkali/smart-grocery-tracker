...
# Usage: ./scripts/start/start-cluster.sh  (run from project root)
#
...
echo ""
echo "⚠️  Remember to run ./scripts/stop/stop-cluster.sh when you're done!"
echo ""
...
Remember to run ./scripts/stop/stop-cluster.sh when you are done to avoid charges." \
    --region us-east-2 > /dev/null
fi
